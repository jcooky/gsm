...
export class KnowledgeGraphManager {
  ...
  async createRelations(relations: Relation[]): Promise<Relation[]> {
    ...
    for (const rel of relations) {
      const { data: fromEntity } = await this.supabase
        ...
        .eq("namespace_id", namespaceId).eq("name", rel.from).maybeSingle()

      const { data: toEntity } = await this.supabase
        .from("entities").select("id")
        .eq("namespace_id", namespaceId).eq("name", rel.to).maybeSingle()
  ...
  async addObservations(
    ...
    for (const obs of observations) {
      const { data: entity, error: entityError } = await this.supabase
        .from("entities").select("id")
        .eq("namespace_id", namespaceId).eq("name", obs.entityName).maybeSingle()
      ...
      const { data: existing } = await this.supabase
        .from("observations").select("content").eq("entity_id", entity.id)

      const existingContents = new Set((existing ?? []).map((o: { content: string }) => o.content))
  ...
  async deleteEntities(entityNames: string[]): Promise<void> {
    ...
    await this.supabase
      ...
      .eq("namespace_id", namespaceId).in("name", entityNames)
  }
  ...
  async deleteObservations(deletions: { entityName: string; observations: string[] }[]): Promise<void> {
    ...
    for (const d of deletions) {
      const { data: entity } = await this.supabase
        .from("entities").select("id")
        .eq("namespace_id", namespaceId).eq("name", d.entityName).maybeSingle()
      if (!entity) continue
      await this.supabase.from("observations").delete()
        .eq("entity_id", entity.id).in("content", d.observations)
    }
  }
  ...
  async deleteRelations(relations: Relation[]): Promise<void> {
    ...
    for (const rel of relations) {
      ...
      const { data: toEntity } = await this.supabase
        ...
        .eq("namespace_id", namespaceId).eq("name", rel.to).maybeSingle()
      if (!fromEntity || !toEntity) continue
      await this.supabase.from("relations").delete()
        .eq("namespace_id", namespaceId)
  ...
  async openNodes(names: string[]): Promise<KnowledgeGraph> {
    ...
    const { data: entities } = await this.supabase
      .from("entities")
      .select("id, name, entity_type, observations(content)")
      .eq("namespace_id", namespaceId).in("name", names)
  ...
  private async entitiesToGraph(
    ...
    return {
      entities: entities.map((e) => ({
        ...
        observations: e.observations.map((o: { content: string }) => o.content),
      })),
